...
添加创意中，==直投就直接投直播间画面==，投素材则选视频。视频的创意类型有两种：自定义创意和程序化创意（[[基础/千川投手/note#^od4tin|二者的区别]]）
...
盯盘用两个页面：
- 一个是 [[基础/千川投手/note#^uidg5m|这个页面]]（看当天的数据，【竞价推广】-【标准推广】）
- 另一个是下图这个（看当场直播的数据，【数据】-【直播分析】）![[Pasted image 20241125154418.png]]

> 大部分时候用的还是 [[基础/千川投手/note#^uidg5m|看当天页面]]，因为这个能看时段，另一个不能
...
搜索流量分析（搜索词）后续讲，这个里面的标签根据你的行业产品情况可选用到你的 [[基础/千川投手/note#^s4d82p|创意标签]] 中
...
[[基础/千川投手/note#^fuhsgs|老客日常占比在 10-20%，如果超过 20%（占比过高）]]可以在新建计划中选**新客转化**

![[Pasted image 20241221161838.png]]

新客转化一般新户是不勾选，因为新户没有任何模型数据去支撑探索，但也有个别新户刚开始就打新客，转化还不错

[[基础/千川投手/note#^jwnm68|直播加热]]（小放量）：转化目标为浅层（一般我们不跑，除非老板要求），商点、互动、下单都是直播加热的转化目标，冲人气用。但日常就算跑浅层，也更倾向用随心推，**因为随心推的浅层比千川便宜**
...
[[基础/千川投手/note#^a0wjme|固定时长]] （也叫快速推）算小放量，一般我们设置 6 个小时
...
**计划名称**可这么命名：日期+ [[基础/千川投手/note#^1t9ndv|转化目标]] +视频/直投+定向+创意分类

![[Pasted image 20241221180640.png]]

 [[基础/千川投手/note#广告组|广告组]]一般可分为三个：
- 优质复制组
- 放量组
- 测试素材组
...
> [!important]
> 同定向的情况下，托管出价可以比自定义低，[[基础/千川投手/note#^3qqejm|原因]]
...
[[基础/千川投手/note#^ebnizs|ecpm]] 越高，[[基础/千川投手/note#^p6ap66|用户刷到的概率越高]]
...
成交探索（A1-A5 人群），它会优先触达 A1-A3，因为**成交的逻辑是以成交为目的去给你探索更容易成交的人**。 ^kq1tal
...
新户先打成交积累足够多的订单量（[[基础/千川投手/note#^kq1tal|先打成交的原因]]），打的差不多再介入 ROI，在此期间不要打托管（[[基础/千川投手/note#^giijlu|原因]]，托管权重大，打托管会影响成交和 ROI 计划）
