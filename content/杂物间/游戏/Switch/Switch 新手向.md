...
破解游戏下载教学（DBI）：打开【相册-DBI】，使用有数据传输功能的 Type-C 数据线连接 Switch 和电脑，将游戏安装程序文件下载至 `MicroSD Install` 目录中（下载过程会自动安装），可连续拖拽多个游戏文件下载（队列等待），下载完成安装文件会被自动删除，游戏具体下载地址不用理会（[[Switch 新手向#^2z2cjr | 游戏卸载教程]]）下载顺序依次为：本体-->补丁-->DLC，DLC 为游戏扩展内容，不是独立出来的番外，所以需要先安装游戏本体，同时补丁我们只需要下载**最新版**即可
...
Switch 游戏存档和游戏本身是两个存储目录，即使[[Switch 新手向#^2z2cjr | 删除游戏]]，在当前主机，该游戏存档仍在。
